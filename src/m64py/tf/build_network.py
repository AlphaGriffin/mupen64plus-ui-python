...
class MupenNetwork(object):
    ...
    def build_network(self):
        ...
        self.x = tf.placeholder(
            tf.float32, shape=[None, self.IMG_H, self.IMG_W, 3])
        self.y_ = tf.placeholder(
            tf.float32, shape=[None, self.OUT_SHAPE])
        self.x_image = self.x
        ...
        self.h_conv1 = tf.nn.relu(
            self.conv2d(self.x_image, self.W_conv1, 2) + self.b_conv1)
        ...
        self.h_fc1 = tf.nn.relu(
            tf.matmul(self.h_conv5_flat, self.W_fc1) + self.b_fc1)
        ...
        self.h_fc2 = tf.nn.relu(
            tf.matmul(self.h_fc1_drop, self.W_fc2) + self.b_fc2)
        ...
        self.h_fc3 = tf.nn.relu(
            tf.matmul(self.h_fc2_drop, self.W_fc3) + self.b_fc3)
        ...
        self.h_fc4 = tf.nn.relu(
            tf.matmul(self.h_fc3_drop, self.W_fc4) +
            self.b_fc4)
        ...
        self.loss = tf.reduce_mean(
            tf.square(tf.subtract(self.y_, self.y))) +\
            tf.add_n([tf.nn.l2_loss(v) for v in self.train_vars]) *\
            0.001
        tf.add_to_collection('loss', self.loss)

        self.train_step = tf.train.AdamOptimizer(.001).minimize(
